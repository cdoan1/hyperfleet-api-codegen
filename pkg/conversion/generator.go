...
func (g *Generator) generateProjectFunction(resource string) string {
	...
	b.WriteString("\t\tID:     string(crd.UID),\n")
...
func (g *Generator) generateProjectSpecFunction(resource, specType string) string {
	...
	for _, fi := range ti.Fields {
		...
		// Handle passthrough types specially
		if strings.Contains(fi.GoType, "Passthrough") {
			b.WriteString(fmt.Sprintf("\t\t%s: project%s(crd.%s),\n", fi.GoName, fi.GoType, fi.GoName))
		} else {
...
func (g *Generator) generatePassthroughHelpers(specType string) string {
	...
	// Find passthrough fields
	for _, fi := range ti.Fields {
		if !strings.Contains(fi.GoType, "Passthrough") {
			continue
		}

		passthroughType := fi.GoType
		pti, exists := g.typeInfos[passthroughType]
		if !exists {
			continue
		}

		// Generate project helper
		b.WriteString(fmt.Sprintf("// project%s converts CRD passthrough to REST\n", passthroughType))
		...
		b.WriteString(fmt.Sprintf("// unproject%s converts REST passthrough to CRD\n", passthroughType))
