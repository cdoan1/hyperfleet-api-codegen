...
[![Coverage](https://img.shields.io/badge/coverage-44%25-green)](https://github.com/cdoan1/hyperfleet-api-codegen)
...
| File | Description | Generated By | Usage |
|------|-------------|--------------|-------|
| `api/v1alpha1/hostedclusterspec.passthrough.go` | Passthrough types mirroring HyperShift HostedClusterSpec and NodePoolSpec with Go markers | `make generate-passthrough` | Edit to curate field visibility and write-modes. Source of truth for field markers. |
| `pkg/registry/field_metadata.go` | Go registry mapping field paths to metadata (write-mode, feature gates, visibility) | `make generate-registry` | Used by runtime validator and consumed by Go code. Enables marker preservation during passthrough regeneration. |
| `pkg/registry/field_metadata.json` | JSON version of field metadata registry | `make generate-registry` | Used by passthrough generator to preserve markers on regeneration. Also consumed by tooling. |
| `openapi/openapi.json` | OpenAPI 3.0 schema for the HyperFleet Platform API | `make generate-openapi` | Powers Swagger UI and API documentation. Respects visibility markers from passthrough types. |

**Regeneration workflow:**
1. Edit passthrough types → `make generate-registry` → updates both registry files
2. Registry changes → `make generate-openapi` → updates OpenAPI schema
3. All three: `make generate` (runs all generators in correct order)
...
./config
  ...
  - bases/ - Base CRD YAML files generated by controller-gen
    - _clusters.yaml - Cluster CRD
    - _nodepools.yaml - NodePool CRD
  - variants/ - Feature-set-specific CRD variants generated by crd-variants tool
    - cluster_default.yaml - Default (GA only) variant
    - cluster_techpreview.yaml - TechPreview variant
    - cluster_devpreview.yaml - DevPreview variant
```
